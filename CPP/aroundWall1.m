function [] = aroundWall1(clientID, vrep, States, dis, left_Motor, right_Motor)
    if (States(1) ==1 &&  States(3) ~= 1 && States(4) ~= 1)%sum(States) == 1 && 
        if(norm(dis(1))<0.5)
    ...
    elseif (States(1) == 1 && States(3) == 1 && States(4) == 1 )
            [~] = vrep.simxSetJointTargetVelocity(clientID,left_Motor,0.5, vrep.simx_opmode_oneshot);
            [~] = vrep.simxSetJointTargetVelocity(clientID,right_Motor,-0.5,vrep.simx_opmode_oneshot);
            pause(0.7)     
    elseif (States(3) == 1 && States(4) == 1 && States(1) ~= 1)
        if(dis(3)>0.5 && dis(4)>0.5)
            [~] = vrep.simxSetJointTargetVelocity(clientID,left_Motor,0.5, vrep.simx_opmode_oneshot);
            [~] = vrep.simxSetJointTargetVelocity(clientID,right_Motor,-0.5,vrep.simx_opmode_oneshot);
        elseif(dis(3)<0.5&& dis(4)<0.5 && (dis(3)- dis(4)<-0.002))
